...
import { nutritionSchema } from "@/lib/validation/plans";
...
export async function POST(request: Request, { params }: Ctx) {
  ...
  const parsed = nutritionSchema.safeParse(await request.json().catch(() => null));
  if (!parsed.success) {
    return NextResponse.json({ error: "invalid" }, { status: 400 });
  }
  const { title, notes, meals } = parsed.data;

  const plan = await prisma.nutritionPlan.create({
    data: { trainerId, clientId, title, data: { notes, meals } },
  });
